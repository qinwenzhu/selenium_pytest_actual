...
class UserPage(BasePage):
    ...
    def add_department_by_root_name(self, dep_name="Default", flag=True):
        """
        从根部门 - Default 下创建分组
    ...
    def create_department_group(self, til_name, dep_name, confirm=True):
        """
        方法封装：用于创建 同级/下一级 分组
        :param til_name: dialog弹框中的标题
        ...
        BasePage(self.driver).update_input_text(INPUT_TEXT, dep_name)
    ...
    def delete_dep_by_name(self, dep_name, confirm=True):
        GroupTree(self.driver).click_menu_by_name(dep_name, "删除")
        if confirm:
            # 点击确认
            CONFIRM_BTN = (By.XPATH, '//span[contains(text(),"删除")]/parent::div/following-sibling::div[@class="el-dialog__footer"]//span[contains(text(),"确定")]')
            BasePage(self.driver).click_ele(CONFIRM_BTN)
        else:
            # 点击取消
            CONFIRM_BTN = (By.XPATH,
