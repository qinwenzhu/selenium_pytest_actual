...
class GroupTree(BasePage):
    ...
    def click_group_by_name(self, department_name):
        ...
        BasePage(self.driver).click_ele(DEPARTMENT_NAME, "左侧组件-tree")
    ...
    def click_menu_by_name(self, department_name, menu_name):
        ...
        BasePage(self.driver).mouse_move_ele(GROUP_ICON, "左侧组件-tree")
    ...
    def create_dep_group_com(self, group_name, loc_by_til_name, confirm=True):
        ...
        BasePage(self.driver).update_input_text(GROUP_INPUT, group_name, "左侧组件-tree")
        if confirm:
            # 点击确认按钮
            ...
            BasePage(self.driver).click_ele(CONFIRM_BTN, "左侧组件-tree")
        else:
            # 点击取消按钮
